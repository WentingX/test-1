	...
	export default {
		...
		methods: {
			...
			messageOrder(params) {
				...
				this.$data.interface.apiname = 'messageOrder';
				this.$data.interface.params = params; //保存接口所用参数，分页要用
				API.messageOrder(params).then(function(res) {
					if(res.code == 200) {
			...
			messageSearch() {
				...
				this.$data.interface.apiname = 'messageSearch';
				API.selXXAllEmployeeOder(par).then(function(res) {
					if(res.code == 200) {
						_this.$data.tabeldata = res.data.list;
						//_this.$data.pagetotal = res.data.total - 0;

...
<div class="modificationHomeBar">
    <div class="grid-content bg-purple-dark">| 投后管理banner管理</div>
            ...
            onBtnPreserveClick(formLabelAlign) {
                this.$refs[formLabelAlign].validate((valid) => {
                    if (valid) {
                        let vm = this
                        vm.loading = true
                        let parameter = 'id=' + this.$route.path.split('/')[4] + '&name=' + this.formLabelAlign.name + '&imageUrl=' + this.dialogImageUrl + '&url=""&sort=' + this.formLabelAlign.sort + '&type=4'
                        API.amendBanner(parameter).then(function(result) {
                            ...
                            if (result.code == 200) {
                                ...
                                vm.$router.push({
                                    path: `/showHomeManagement/investmentBar`
                                });
                            } else {
                                vm.$message.error({
                                    message: '修改失败',

        ...
        created() {
            ...
            API.bannerInfo(params).then(function(result) {
                ...
                if (result.code == 200) {
                    ...
                    vm.formLabelAlign.src = result.data.url
                } else {
                    vm.$message.error({
                        message: '数据加载失败',
                        showClose: true,
            ...
            onBtnPreserveClick(formLabelAlign) {
                this.$refs[formLabelAlign].validate((valid) => {
                    if (valid) {
                        let vm = this
                        vm.loading = true
                        let parameter = 'id=' + this.$route.path.split('/')[4] + '&name=' + this.formLabelAlign.name + '&imageUrl=' + this.dialogImageUrl + '&url=""&sort=' + this.formLabelAlign.sort + '&type=7'

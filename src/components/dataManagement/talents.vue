		...
		<div v-else>
			...
			<div class="listS">
				...
				<el-table :data="tableData" v-loading="loading" style="width: 100%;" class="taletableS" align="center" :show-header="false" @row-click="rowClick">
					...
					<el-table-column prop="delFlag" label="下载/查看" width="100" align="right"></el-table-column>
				</el-table>
    ...
    export default {
        ...
        methods: {
            ...
            getInvestmentList(paramStr=this.pageNum){
                ...
                API.resourceLibraryDateStartTimeEndTimeRenCai(deparam2).then( result => {
                    vm.loading = false
                    if(result.code == 200 && result.data != null){
                        this.total = parseInt(result.data.total)
                        this.tableData = result.data.list
            ...
            getInvestmentList2(paramStr=this.pageNum){
                ...
                API.resourceLibraryTimeSelectDataRenCai(parms).then( result =>{
                    if(result.code == 200 && result.data != null){

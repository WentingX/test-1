	...
	<div>
		<el-form :inline="true" class="demo-form-inline docuform" @submit.native.prevent v-if="showOn">
			...
			<el-form-item class="docuuu">
				<el-col :span="20">
					<el-input size="mini" v-model="impKeyWord" placeholder="输入你想搜索的内容" class="dddddd" ></el-input>
				</el-col>
				<el-col :span="4">
		...
		<div v-else>
			...
			<div class="listS">
				...
				<!--表格-->
				<el-table :data="tableData" v-loading="loading" style="width: 100%;" class="taletableS" align="center" :show-header="false">
					<el-table-column prop="annexUrl" label="标题" width="560" align="left"></el-table-column>
					<el-table-column prop="updateDate" label="时间" width="170" align="right"></el-table-column>
					<el-table-column prop="address" label="地区" width="120" align="right"></el-table-column>
					<el-table-column prop="STATUS" label="下载/查看" width="100" align="right">
        ...
        methods:{
            ...
            handleDownload(row) {
                ...
                import ('@/vendor/Export2Excel').then(excel => {
                    const tHeader = ['标题', '时间', '地区']
                    const filterVal = ['annexUrl', 'updateDate', 'address']
                    const data = this.formatJson(filterVal, hh)
                    excel.export_json_to_excel(tHeader, data, '该文件库信息表格')
            ...
            dsearchInfo2(){
                ...
                API.fileLibrary(talentDa).then( result => {
                    if(result.code == "200" && result.data != null){
                        this.total = parseInt(result.data.total)
                        this.tableData = result.data.list
                    }
            ...
            getInvestmentList(paramStr=this.pageNum){
                ...
                API.resourceLibraryDateStartTimeEndTimeWenJian(deparam2).then( result => {
                    vm.loading = false
                    if(result.code == 200 && result.data != null){
                        this.total = parseInt(result.data.total)
                        this.tableData = result.data.list
            ...
            getInvestmentList2(paramStr=this.pageNum){
                ...
                API.resourceLibraryTimeSelectDataWenJian(parms).then( result =>{
                    if(result.code == 200 && result.data != null){

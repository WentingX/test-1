...
<template>
	<div>
		<el-form :inline="true" name="ruleForm" ref="ruleForm" :model="formInline" class="demo-form-inline mesform">
			<p class="title"><em>|</em> 信息方管理</p>
			<el-form-item class="mesuu" prop="user">
				<el-input size="mini" v-model="keyWord" placeholder="输入你想搜索的内容"
				          class="messearchInput"></el-input>
			</el-form-item>
			<el-form-item class="mesbtn">
				<el-button icon="el-icon-search" size="mini" class="mesbInput"
				           @click="dsearchInfo">搜索
				</el-button>
			</el-form-item>
			<br/>
			<el-form-item class="mesname" label="用户名" prop="uerName">
				<el-input size="mini" v-model="formInline.uerName"></el-input>
			</el-form-item>
			<el-form-item class="mesname" label="真实姓名" prop="name">
				<el-input size="mini" v-model="formInline.name"></el-input>
			</el-form-item>
			<el-form-item class="mesname" label="企业名称" prop="companyName">
				<el-input size="mini" v-model="formInline.companyName"></el-input>
			</el-form-item>
			<el-form-item class="mesqq" label="用户类型" prop="type">
				<el-select size="mini" v-model="formInline.type">
					<el-option label="个人" value="01"></el-option>
					<el-option label="企业" value="02"></el-option>
				</el-select>
			</el-form-item>
			<el-form-item class="mesqq" label="信用等级" prop="level">
				<el-select size="mini" v-model="formInline.level">
					<el-option label="区域一" value="shanghai"></el-option>
					<el-option label="区域二" value="beijing"></el-option>
				</el-select>
			</el-form-item>
			<el-form-item class="mesqq" label="账户状态" prop="state">
			...
			<div style="text-align:center;" class="mescent">
				<el-button size="mini" class="mesbuttonInput mesbuttonInput_left" @click="onSubmit">查询</el-button>
				<el-button size="mini" class="mesbuttonInput" @click="resetForm('ruleForm')">重置</el-button>
			</div>
		</el-form>
		<hr class="hrLinek"/>
		<div class="mestaS">
			<p class="title"><em>|</em> 信息方列表</p>
			<el-table :data="tableData" v-loading="loading" style="width: 100%;text-align:center;color:#556191;" :row-class-name="tableRowClassName">
				...
				<el-table-column label="操作" align="center" property="status">
					<template slot-scope="scope">
						<el-button size="mini" type="text" class="mesview"
						           @click="onBtnDetailClick(scope.row)" property="id">详情
						</el-button><!-- v-if="detai"  ,forb  ,star--><!--0启用 -1禁用 反了-->
						<el-button v-if="scope.row.status == '0'" size="mini" type="text" class="mesdisabled"
    ...
    export default {
        ...
        methods: {
            ...
            getInvestmentList (paramStr=this.pageNum) {
                let vm = this
	            vm.loading = true
                API.message(paramStr).then( result => {
                    vm.loading = false
                    if(result.code == "200" && result.data != null){
...
<style lang="scss" scope>
	...
	.mesform .title, .mestaS .title {
		color: #858DAD;
		margin-top: 0;
		font-size:16px;
	}
	.mesform .title em, .mestaS .title em {
		color: #88B7E0;
		margin-right: 10px;
	}
	.mesform .el-input__inner{
		border-radius:0px;
	}
	.mesuu{
		margin-top:20px;
	...
	.mesuu .el-form-item__content,
	.mesbtn .el-form-item__content{
		line-height:normal;
	}
	.mesuu .el-input__inner{
		width: 400px;
		height: 35px;
	...
	.mesbtn .el-form-item__content button {
		width:100px;
		height:35px;
		border:1px solid #999999;
		border-radius:0px;
	...
	.mesname .el-form-item__content {
		width: 140px;
	}

	.mesqq .el-form-item__content {
		width: 120px;
	}
	...
	.mesform .el-button, .mescent .el-button {
		background: #4A4F63;
		color: white;
		padding: 7px 20px;
	}
	...
	.mesform .mesbuttonInput{
		width:126px;
		height:35px;
	}
	.mesform .mesbuttonInput_left{
		margin-right:70px;
	}
	.mestaS {
		margin-top: 10px;

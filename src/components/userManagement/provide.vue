...
<template>
	<div>
		<el-form :inline="true" name="ruleForm" ref="ruleForm" :model="formInline" class="demo-form-inline proform">
			<p class="title"><em>|</em> 服务商管理</p>
			<el-form-item class="prouu" prop="user">
				<el-input size="mini" v-model="keyWord" placeholder="输入你想搜索的内容"
				          class="prosearchInput"></el-input>
			</el-form-item>
			<el-form-item class="probtn">
				<el-button icon="el-icon-search" size="mini" class="probInput"
				           @click="dsearchInfo">搜索
				</el-button>
			</el-form-item>
			<br/>
			<el-form-item class="proname" label="用户名" prop="uerName">
				<el-input size="mini" v-model="formInline.uerName"></el-input>
			</el-form-item>
			<el-form-item class="proname" label="真实姓名" prop="name">
				<el-input size="mini" v-model="formInline.name"></el-input>
			</el-form-item>
			<el-form-item class="proname" label="企业名称" prop="companyName">
				<el-input size="mini" v-model="formInline.companyName"></el-input>
			</el-form-item>
			<el-form-item class="proqq" label="用户类型" prop="type">
				<el-select size="mini" v-model="formInline.type">
					<el-option label="个人" value="01"></el-option>
					<el-option label="企业" value="02"></el-option>
				</el-select>
			</el-form-item>
			<el-form-item class="proqq" label="信用等级" prop="level">
				<el-select size="mini" v-model="formInline.level">
					<el-option label="区域一" value="shanghai"></el-option>
					<el-option label="区域二" value="beijing"></el-option>
				</el-select>
			</el-form-item>
			<el-form-item class="proqq" label="账户状态" prop="state">
			...
			<div style="text-align:center;" class="procent">
				<el-button size="mini" class="probuttonInput probuttonInput_left" @click="onSubmit">查询</el-button>
				<el-button size="mini" class="probuttonInput" @click="resetForm('ruleForm')">重置</el-button>
			</div>
		</el-form>
		<hr class="hrLinek"/>
		<div class="protaS">
			<p class="title"><em>|</em> 服务商列表</p>
			<el-table :data="tableData" v-loading="loading" style="width: 100%;text-align:center;color:#556191;" :row-class-name="tableRowClassName">
				...
				<!--<el-table-column property="credit" label="信用等级" align="center"></el-table-column>-->
				...
				<el-table-column label="操作" align="center" property="status">
					<template slot-scope="scope">
						<el-button size="mini" type="text" class="proview"
						           @click="onBtnDetailClick(scope.row)" property="id">详情
						</el-button><!-- v-if="detai"  ,forb  ,star--><!--0启用 -1禁用 反了-->
						<el-button v-if="scope.row.status == '0'" size="mini" type="text" class="prodisabled"
						           @click="userDisa(scope.row)"
						           property="id">禁用</el-button>
						<el-button v-if="scope.row.status == '-1'" size="mini" type="text" class="greendisabled"
						           @click="userDisa(scope.row)"
						           property="id">启用</el-button>
					</template>
				</el-table-column>
			</el-table>
    ...
    export default {
        ...
        methods: {
            ...
            getInvestmentList (paramStr=this.pageNum) {
                let vm = this
                vm.loading = true
                API.provide(paramStr).then( result => {
                    vm.loading = false
                    if(result.code == "200" && result.data != null){
...
<style lang="scss" scope>
	...
	.proform .title, .protaS .title {
		color: #858DAD;
		margin-top: 0;
		font-size:16px;
	}
	.proform .title em, .protaS .title em {
		color: #88B7E0;
		margin-right: 10px;
	}
	.proform .el-input__inner{
		border-radius:0px;
	}
	.prouu{
		margin-top:20px;
	...
	.prouu .el-form-item__content,
	.probtn .el-form-item__content{
		line-height:normal;
	}
	.prouu .el-input__inner{
		width: 400px;
		height: 35px;
	...
	.probtn .el-form-item__content button {
		width:100px;
		height:35px;
		border:1px solid #999999;
		border-radius:0px;
	...
	.proname .el-form-item__content {
		width: 140px;
	}

	.proqq .el-form-item__content {
		width: 120px;
	}
	...
	.proform .el-button, .procent .el-button {
		background: #4A4F63;
		color: white;
		padding: 7px 20px;
	}
	...
	.proform .probuttonInput{
		width:126px;
		height:35px;
	}
	.proform .probuttonInput_left{
		margin-right:70px;
	}
	.protaS {
		margin-top: 10px;
